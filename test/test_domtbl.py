...
def test_domtbl(datadir: Path):
    tbl = iter(read_domtbl(datadir / "domtbl.txt"))
    row = next(tbl)
    ...
    assert len(read_domtbl(datadir / "domtbl.txt")) == 8


def test_domtbl_empty(datadir: Path):
    tbl = iter(read_domtbl(datadir / "empty.txt"))
